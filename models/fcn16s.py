...
class FCN16s(nn.Module):
    ...
    def __init__(self, n_class=1, drop2d=False, dropout_ratio=0.5):
        ...
        self.score_sal = nn.Conv2d(4096, n_class, 1, padding=0)
        self.score_pool4 = nn.Conv2d(512, n_class, 1, padding=0)

        self.upscore_sal2 = nn.ConvTranspose2d(n_class, n_class, 4, stride=2, bias=False)
    ...
    def _initialize_weights(self):
        for k, v in self._modules.items():
            if k in ["upscore_sal2", "upscore16"] and isinstance(v, nn.ConvTranspose2d):
    ...
    def forward(self, x):
        ...
        h = self.score_sal(h)
        h = self.upscore_sal2(h)
        upscore_sal2 = h  # 1/16

        h = self.score_pool4(pool4)
        h = h[:, :, 5:5 + upscore_sal2.size()[2], 5:5 + upscore_sal2.size()[3]]
        score_pool4c = h  # 1/16

        h = upscore_sal2 + score_pool4c # fuse_pool4

        h = self.upscore16(h)
        h = h[:, :, 27:27 + x.size()[2], 27:27 + x.size()[3]].contiguous() # score
